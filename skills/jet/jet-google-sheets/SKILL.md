---
name: jet-google-sheets
description: Skill for interacting with Google Sheets within JET company Google accounts. Read tables, append rows, update cells by key or index, batch operations.
metadata:
  owner: ai-platform
---
# sheets-cli
...
## Usage
Use the `sheets-cli` script in the scripts directory
```bash
./scripts/sheets-cli <command> [options]
```
...
### Auth (Setup)
```bash
sheets-cli auth login --credentials conf/client_secret_602299916251-7aodem5aquraoo8m9goeepn3ugi5ok22.apps.googleusercontent.com.json
